  ...
  return (
    <div className="fixed top-4 right-4 sm:top-6 sm:right-6 z-[9999] max-w-md w-[calc(100vw-2rem)] animate-bounce-in shadow-2xl">
      <div className={`rounded-xl border p-4 backdrop-blur-xl ${
        isAppointment 
          ? 'bg-[#121810]/95 border-[#556b2f] text-stone-100 shadow-[#556b2f]/20' 
        ...
        <div className="flex items-start justify-between gap-3 border-b border-white/10 pb-3 mb-3">
          <div className="flex items-center gap-2.5">
            <div className={`p-2 rounded-lg ${isAppointment ? 'bg-[#556b2f]/30 text-[#556b2f]' : 'bg-yellow-500/30 text-yellow-400'} animate-pulse`}>
              {isAppointment ? <Calendar className="w-5 h-5" /> : <DollarSign className="w-5 h-5" />}
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${
                  isAppointment ? 'bg-[#556b2f] text-black' : 'bg-yellow-500 text-black'
                }`}>
                  ALERTA SONORO ENVIADO
                </span>
                <span className="flex h-2 w-2 relative">
                  <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${isAppointment ? 'bg-[#556b2f]' : 'bg-yellow-400'} opacity-75`}></span>
                  <span className={`relative inline-flex rounded-full h-2 w-2 ${isAppointment ? 'bg-[#556b2f]' : 'bg-yellow-500'}`}></span>
                </span>
              </div>
        ...
        <div className="space-y-2 text-xs font-medium text-stone-300">
          ...
          <div className="flex items-center justify-between bg-black/40 p-2 rounded border border-white/5">
            <span className="flex items-center gap-1.5 text-stone-400">
              {isAppointment ? <Calendar className="w-3.5 h-3.5 text-[#556b2f]" /> : <Sparkles className="w-3.5 h-3.5 text-yellow-400" />} 
        ...
        <div className="mt-4 pt-3 border-t border-white/10 flex items-center justify-between gap-2">
          ...
          <a
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded font-bold text-[11px] transition uppercase tracking-wider ${
              isAppointment
                ? 'bg-[#556b2f] hover:bg-[#6b873b] text-black shadow-lg shadow-[#556b2f]/30'
